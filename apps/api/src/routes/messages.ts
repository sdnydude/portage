...
messagesRouter.get('/', async (req, res, next) => {
  try {
    ...
    const conversations = await db.execute(sql`
      ...
      GROUP BY conversation_key, buyer_username, item_id, item_title
...
messagesRouter.post('/sync', async (req, res, next) => {
  try {
    ...
    try {
      ...
      throw new AppError(400, 'NO_EBAY_ACCOUNT', 'No eBay account connected. Connect your eBay account in Settings > Marketplace.');
    }
