...
describe("resolvePublishMode", () => {
  it("maps publish-panel choices to a POST /listings publishMode", () => {
    ...
    // eBay-draft only applies to eBay; on etsy it falls back to a local draft
    expect(resolvePublishMode({ publishNow: false, ebayDraft: true, marketplace: "etsy" })).toBe("draft");
