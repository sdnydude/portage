...
| `scanning` | `POST /scan/refine` sends R2 URLs to the AI vision provider chain (configured via `VISION_PROVIDERS`; Gemini 3.5 Flash-Lite primary with automatic fallback) for identification. The response's `detailed.provenance` names which provider/model answered, and Save persists it under `marketplaceData.scan.provenance`. |
