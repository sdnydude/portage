...
- Top→bottom: wordmark (icon-only when collapsed), orange **Scan** button (opens existing `ScanFlow` — desktop webcam + gallery both already work), 6 nav items (same set/order as TabBar: Home, Inventory, Listings, Porter, Orders, More), collapse toggle.
- Nav items: icon + label (icon-only + tooltip when collapsed), active state from `usePathname`, Porter keeps the teal accent, unread badge on More (Messages count via `useUnreadCount`).
...
- Keeps: 6 tabs, center orange Scan FAB breaking the top edge, active dots, Porter teal, content fade gradient beneath, safe-area handling.
...
### F. HomeChip — `apps/web/src/components/layout/home-chip.tsx` (`<lg`, non-tab pages)
- Round glass chip (same idiom as the floating bar), home icon, fixed bottom-left at the same x/y the Home tab occupies in the floating bar; links to `/home`.
- Rendered by AppShell; hidden on tab routes and at `lg+`. Full-screen overlays (ScanFlow z-60) naturally cover it (chip sits at z-50).
...
- TabBar: floating classes pin (rounded-[22px], inset), hidden at lg (class assertion), existing behavior tests keep passing.
- HomeChip: shows on non-tab route, absent on tab route, links `/home`.
