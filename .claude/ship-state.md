status: in_progress
phase: 6
resume_at: "PHASE 5 (VERIFY) — fresh session resume. Phase 4 (Build) COMPLETE: all 20 tasks committed on branch feat/ebay-listing-hardening (43 commits ahead of main; includes prior merged PR #93 OAuth work). Working tree clean at session end. RUN THESE, SHOW OUTPUT: (1) `npm test -w apps/api` — EXCLUDE apps/api/src/routes/__tests__/batch-enhance.test.ts (4 PRE-EXISTING fails, untracked photo-gallery WIP, NOT this ship); expect ~344 pass. (2) `npm run typecheck` — 3 ws clean. (3) `npm run lint` — 0 errors / 25 pre-existing warnings. (4) Re-run key task verifies: db columns already pushed+psql-verified (T1 quantity/ebaySku/ebayPublishMode, T13 listings.ebay_offer_id). NEW ENDPOINT POST /seller-profile/ebay/auto-setup (T12) — covered by 4 supertest cases (mocked); live smoke-test needs a CONNECTED eBay account + is sandbox-gated (EBAY_SANDBOX), so perf-baseline only if connected, else cite the supertest coverage. (5) Health: portage-api/app/db containers — NOTE the running app+api containers are ~25h STALE (Docker no hot-reload); they do NOT contain this branch. Sandbox shell CANNOT reach 10.0.0.251:8016/app (returns 000); registry 8011 IS reachable. FRONTEND VISUAL verification (chunks 6-7) is DEFERRED to live review per user — requires `docker compose up -d --build portage-app portage-api` which needs USER APPROVAL (running services). DO NOT rebuild without it. (6) Meta-verify this ship-state matches reality. KNOWN STATE: 1 HIGH deferred item (registry c3b3013c, photo-first fallback publish gap). Registry records this run: ship_session dd9e8b52, deferred c3b3013c, insights 1ec6286a + 9543f3bf, corrections bce7af1c, decision e068fffb. After Phase 5: Phase 6 (Review — 6 agents on git diff main..HEAD) → Phase 7 (Ship — PR + CLAUDE.md update + present defer list). tdd-guard: backend chunks tested; apps/web exempt. KEEP gate ON."
...
phase4_progress:
  ...
  current: "PHASE 6 (REVIEW) COMPLETE. 10 review agents dispatched (6 standard + 4 adversarial). 6 Critical bugs fixed: C1 Zod ebayPublishMode stripped, C2 null profile crash, C3 TOCTOU userId guard, C4 fulfillmentPolicy missing shippingCarrierCode, C5 XSS via eBay longMessage, C6 quantity min(0). 3 Important fixes: I1 empty offer PUT guard, I5 merchantLocationKey validation, I6 comment accuracy. 2 false positives disproven (bulkPublishOffers chunking, TOCTOU claims). Tests: 350/350 pass (+5 new). Typecheck: 3 ws clean. Lint: 0 errors/25 pre-existing warnings. Fix commit: a8923e6. DEFERRED from review: I2 orphaned inventory_item cleanup, I3 type gap packages/shared, I4 bulk activate response, type design concerns. NEXT: Phase 7 (Ship — PR + CLAUDE.md update + defer list)."
