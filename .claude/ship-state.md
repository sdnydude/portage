status: in_progress
phase: 4
resume_at: "Phase 4 (Build) IN PROGRESS. T1-T8 done + committed (8/20) on branch feat/ebay-listing-hardening. CHUNK 1 (T1-T3) + CHUNK 2 (T4-T6) COMPLETE; CHUNK 3 (T7-T9): T7 (1a0f73c) + T8 (e935163) done. Resume at T9 (surface eBay longMessage on 400). T9 REALITY: adapter request() (ebay-adapter.ts ~166-188) flattens errors into 'eBay API error: {status} on {path}' and DISCARDS the parsed body — T9 must parse errors[].longMessage onto the thrown error first, then surface it; tdd-able in the adapter harness. Run-to-completion TDD rhythm; tdd-guard STRICT (each impl edit must ADDRESS the failing assertion; type-only adds against `as any` tests may need to land on green — seen T7 + T8, it is the backlogged false-positive). ROUTE PUBLISH LAYER consolidated in T13: T7 ebaySku/quantity persistence + T8 POST /:id/publish reuse wiring + publishMode routing/validation + add a new listings.ebayOfferId column (live publish overwrites marketplaceListingId with the listingId, losing the offerId). Next after T9: Chunk 4 (T10-T12 auto-setup). Settings on auto mode + curated allowlist."
...
phase4_progress:
  ...
  current: T9 (surface eBay longMessage on 400 — chunk 3). T9 REALITY: adapter request() (ebay-adapter.ts ~166-188) flattens errors into "eBay API error: {status} on {path}" and DISCARDS the parsed body — T9 must parse errors[].longMessage onto the thrown error first, then surface it (tdd-able in the adapter harness). NOTE: T13 owns the full route publish layer — ebaySku/quantity (T7) + POST /:id/publish reuse wiring (T8) + publishMode/validation + a new listings.ebayOfferId column.
