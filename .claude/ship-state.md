status: in_progress
phase: 4
resume_at: "Phase 4 (Build) IN PROGRESS. T1-T9 done + committed (9/20) on branch feat/ebay-listing-hardening. CHUNK 1 (T1-T3) + CHUNK 2 (T4-T6) + CHUNK 3 (T7-T9) COMPLETE. Resume at T10 (Chunk 4 auto-setup): adapter methods createFulfillmentPolicy / createPaymentPolicy / createReturnPolicy via eBay Account API — TDD in the ebay-adapter.test.ts fetch-mock harness; mirror the existing static-method pattern and assert the POST body. Run-to-completion TDD rhythm; tdd-guard STRICT (each impl edit must ADDRESS the failing assertion; type-only adds against `as any` tests land on green — seen T7+T8, the backlogged false-positive). DEFERRED to T13 (route publish layer): T7 ebaySku/quantity persistence + T8 POST /:id/publish reuse wiring + publishMode routing/validation + ADD a listings.ebayOfferId column (live publish overwrites marketplaceListingId with the listingId, losing the offerId). Chunks remaining: 4 = T10-T12 (auto-setup), 5 = T13-T15 (route hardening), 6 = T16-T18 (frontend), 7 = T19-T20 (settings UI). Settings on auto mode + curated allowlist."
...
phase4_progress:
  ...
  current: T10 (Chunk 4 auto-setup — adapter methods createFulfillmentPolicy / createPaymentPolicy / createReturnPolicy via eBay Account API). CHUNK 3 (T7-T9) COMPLETE. NOTE: T13 owns the full route publish layer — ebaySku/quantity (T7) + POST /:id/publish reuse wiring (T8) + publishMode/validation + a new listings.ebayOfferId column.
