...
phase4_progress:
  ...
  current: T13 (Chunk 5 route hardening — POST /listings: add publishMode routing + validate eBay marketplaceSpecificFields, persist ebaySku + forward item.quantity to createListing [route glue deferred from T7], wire POST /:id/publish reuse [deferred from T8: pass listing.ebaySku + offerId-from-marketplaceListingId into createListing], ADD a listings.ebayOfferId column [live publish overwrites marketplaceListingId with listingId, losing the offerId]). CHUNK 4 (T10-T12) COMPLETE (1b92db7, 7453445, 36c65a0). HEED tdd_guard_ROUTE_LESSON above — T13-T15 are route tasks. NOTE: T13 owns the full route publish layer — ebaySku/quantity (T7) + POST /:id/publish reuse wiring (T8) + publishMode/validation + a new listings.ebayOfferId column.
