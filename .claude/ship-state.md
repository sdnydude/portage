...
phase4_progress:
  ...
  current: "T20 (Chunk 7 — Settings UI, tdd-guard EXEMPT, FINAL build task). T19 DONE (e05b494): auto-setup button + 3-state status indicator. T20 = publish mode default selector on same page (apps/web/src/app/settings/seller-profile/page.tsx). Add a control bound to profile.ebayPublishMode ('draft'|'live', schema default 'live' per T1) via updateField('ebayPublishMode', v). SellerProfile.ebayPublishMode type exists (T2). autoPublish checkbox is SEPARATE (stays). Place in eBay Account section or Listing Preferences. Verify: dev server change mode → reload → persisted. >>> CHUNK 7: T19 DONE, T20 next (LAST build task) <<< After T20: Phase 5 (Verify) → 6 (Review) → 7 (Ship)."
