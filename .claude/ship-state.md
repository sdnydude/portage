...
phase4_progress:
  ...
  current: "T17 (Chunk 6 — frontend wiring, tdd-guard EXEMPT). T16 DONE (e7c5753): useListingFlow.publish() accepts {ebayPreparedFields, publishMode} params + ListingFlowState.quantity. T17 = wire the 3 flow components (hybrid-flow.tsx, conversational-flow.tsx, swipe-flow.tsx) + ListingPreviewCard to: (a) pass ebayPreparedFields (from usePrepareListing) + publishMode into publish(), (b) add a quantity input bound to setField('quantity'), (c) add a draft/live toggle on ListingPreviewCard. Verify: dev server — quantity visible, toggle works, publish sends fields. T18 = inventory edit page quantity field. >>> CHUNK 6: T16 DONE, T17 next <<< Chunks remaining: 6 = T17-T18 (frontend), 7 = T19-T20 (Settings UI)."
