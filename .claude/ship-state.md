...
phase4_progress:
  ...
  current: T14 (Chunk 5 — PATCH /:id updateListing: sync ALL fields to eBay, i.e. inventory_item (condition/aspects/weight/quantity via PUT) + offer (price/title/desc) — currently updateListing only PUTs the offer. Files: listings.ts + ebay-adapter.ts. Use listings.test.ts harness (adapter mocked via vi.hoisted) + ebay-adapter.test.ts fetch-mock harness. tdd:true, route task → HEED tdd_guard_ROUTE_LESSON.). T13 DONE (eb028b7 schema, b883433 route). Then T15 = POST /bulk/activate bulkPublishOffers." — ebaySku/quantity (T7) + POST /:id/publish reuse wiring (T8) + publishMode/validation + a new listings.ebayOfferId column.
