...
phase4_progress:
  ...
  current: "T19 (Chunk 7 — Settings UI, tdd-guard EXEMPT). CHUNK 6 (T16-T18) COMPLETE. T19 = Settings/seller-profile: 'Set up eBay Selling' button (calls POST /seller-profile/ebay/auto-setup from T12) + status indicator (green when 4 policy/location IDs present). File: apps/web/src/app/settings/seller-profile/page.tsx. No useSellerProfile hook — page uses inline api() calls (per phase3 revision). autoPublish checkbox stays; ebayPublishMode (T20) is separate. T20 = publish mode default selector on same page. >>> CHUNK 6 COMPLETE, T19 next <<< Chunks remaining: 7 = T19-T20 (Settings UI). Then Phase 5 (Verify) → 6 (Review) → 7 (Ship)."
