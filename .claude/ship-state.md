status: in_progress
phase: 4
resume_at: "Phase 4 (Build) IN PROGRESS. T1-T6 done + committed (6/20) on branch feat/ebay-listing-hardening. CHUNK 1 (T1-T3) + CHUNK 2 (T4-T6) COMPLETE. Resume at T7 (conditional publish — draft mode skips /publish, stores offerId + SKU — chunk 3). Read phase4_progress + tdd_guard_DECISION at end of file BEFORE building; follow the run-to-completion TDD rhythm (tdd-guard is strict: stub-first, one test per edit, edits must relate to the current failing test). Next after T7: T8 (SKU reuse), T9 (surface eBay longMessage). Settings on auto mode + curated allowlist (verified working this session)."
...
phase4_progress:
  ...
  current: T7 (conditional publish — draft mode skips /publish, stores offerId + SKU — chunk 3)
