...
- **Item scanning:** configurable provider chain defined by the `VISION_PROVIDERS` env var, consumed in `apps/api/src/lib/ai-client.ts` (`apps/api/src/lib/vision.ts` holds the vision prompt/schema logic). Entry syntax `provider[:model][@reasoning_effort]`. Approved chain 2026-09-05: `gemini:gemini-3.5-flash-lite@minimal,gemini:gemini-3.8-flash,anthropic:claude-haiku-4-5` (local qwen3-vl removed — 43% prefill JSON failures, VRAM thrash with Porter's granite). Each scan stamps which provider/model answered the identification and aspect-prefill calls into `items.marketplaceData.scan.provenance` and onto the `Refine scan complete` log line
...
Test suite: 1094 API / 714 web as of 2026-09-07 (post PRs #356-#359); 27 generator as of 2026-08-27.
